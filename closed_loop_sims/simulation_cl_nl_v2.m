...
rmpath('..\required_files\cloth_model_FColtraro')
rmpath('..\required_files\cloth_model_DParent')
...
%{x
% For a 4x4 (Optimized old)
COM.stiffness = [-305.6028  -13.4221 -225.8987]; 
COM.damping = [-4.0042   -2.5735   -3.9090]; 
COM.z_sum = 0.0312; %0.0312;
%}
%{
% For a 4x4 (Optimized new)
COM.stiffness = [-300.38 -13.47 -269.82]; 
COM.damping = [-4.48 -3.08 -4.33]; 
COM.z_sum = 0.0273;
%}
%{
...
%}
%{
% For a 4x4 (Learnt Exp3)
COM.stiffness = [-335.4255  -88.8234 -306.1666]; 
COM.damping = [-3.9707   -2.2933   -3.9416]; 
COM.z_sum = 0.0206;
%}
%{
...
set(gca, 'TickLabelInterpreter', 'latex');
hold on
plot(time,All_StC(coord_l([2 4 6]),:)','--', 'linewidth',1.5);
hold off
legend('$x_{SOM}$','$y_{SOM}$','$z_{SOM}$', ...
       '$x_{COM}$','$y_{COM}$','$z_{COM}$', ...
       'NumColumns',2, 'Interpreter', 'latex')
