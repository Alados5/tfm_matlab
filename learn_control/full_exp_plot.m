...
elseif(plotType == 2)
    ...
    subplot(3,1,1)
    plot(0:NFiles*NEpochs, MW(:,1))
    grid on
    set(gca,'TickLabelInterpreter','latex');
    xlabel('Epoch', 'Interpreter','latex');
    ylabel('$k$ [N/m]', 'Interpreter','latex');
    subplot(3,1,2)
    plot(0:NFiles*NEpochs, MW(:,2))
    grid on
    set(gca,'TickLabelInterpreter','latex');
    xlabel('Epoch', 'Interpreter','latex');
...
elseif(plotType == 4)
    ...
    nTh = size(MW,2);
    SWD = zeros(size(SW,1),1,size(SW,3));
    for i=1:size(SW,3), SWD(:,:,i) = diag(SW(:,:,i)); end
    ...
    set(gca,'TickLabelInterpreter','latex');
    xlabel('Epoch', 'Interpreter','latex');
    ylabel('$\mu$', 'Interpreter','latex','FontSize',11);
    title('\textbf{W1}', 'Interpreter','latex','FontSize',12);
    subplot(2,nTh,2)
    plot(0:NFiles*NEpochs, MW(:,2))
    grid on
    set(gca,'TickLabelInterpreter','latex');
    xlabel('Epoch', 'Interpreter','latex');
    ylabel('$\mu$', 'Interpreter','latex','FontSize',11);
    title('\textbf{W2}', 'Interpreter','latex','FontSize',12);
    if(nTh==4)
    subplot(2,nTh,3)
    plot(0:NFiles*NEpochs, MW(:,3))
    grid on
    set(gca,'TickLabelInterpreter','latex');
    xlabel('Epoch', 'Interpreter','latex');
    ylabel('$\mu$', 'Interpreter','latex','FontSize',11);
    title('\textbf{W3}', 'Interpreter','latex','FontSize',12);
    subplot(2,nTh,4)
    plot(0:NFiles*NEpochs, MW(:,4))
    grid on
    set(gca,'TickLabelInterpreter','latex');
    xlabel('Epoch', 'Interpreter','latex');
    ylabel('$\mu$', 'Interpreter','latex','FontSize',11);
    title('\textbf{W4}', 'Interpreter','latex','FontSize',12);
    end
    
    subplot(2,nTh,nTh+1);
    plot(0:NFiles*NEpochs, SWD(:,1))
    grid on
    set(gca,'TickLabelInterpreter','latex');
    xlabel('Epoch', 'Interpreter','latex');
    ylabel('$\sigma$', 'Interpreter','latex','FontSize',11);
    subplot(2,nTh,nTh+2);
    plot(0:NFiles*NEpochs, SWD(:,2));
    grid on
    set(gca,'TickLabelInterpreter','latex');
    xlabel('Epoch', 'Interpreter','latex');
    ylabel('$\sigma$', 'Interpreter','latex','FontSize',11);
    if(nTh>2)
    subplot(2,nTh,nTh+3);
    plot(0:NFiles*NEpochs, SWD(:,3))
    grid on
    set(gca,'TickLabelInterpreter','latex');
    xlabel('Epoch', 'Interpreter','latex');
    ylabel('$\sigma$', 'Interpreter','latex','FontSize',11);
    subplot(2,nTh,nTh+4);
    plot(0:NFiles*NEpochs, SWD(:,4))
    grid on
    set(gca,'TickLabelInterpreter','latex');
    xlabel('Epoch', 'Interpreter','latex');
    ylabel('$\sigma$', 'Interpreter','latex','FontSize',11);
