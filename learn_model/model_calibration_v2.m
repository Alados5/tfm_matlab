...
nx = SOMsz;
ny = SOMsz;
[SOM, pos] = initialize_nl_model(lCloth,SOMsz,cCloth,Ts);
...
objectiu = 0;
store_u = zeros(6,18*NT);
store_u(:,1:2) = [u_SOM, u_SOM];
for t=3:18*NT
    % Enter the same "u_ini" to both models
    u_lin = u_SOM - u_bef;
    
    xnext_COM = linear_model_for_optimization(x_ini_COM,u_lin,COM);
    [pnext_SOM, vnext_SOM] = simulate_cloth_step(x_ini_SOM,u_SOM,SOM);
    
    u_bef = u_SOM; % Update
    
    x_ini_COM = xnext_COM;
    x_ini_SOM = [pnext_SOM; vnext_SOM];
    
    [reduced_pos,reduced_vel] = take_reduced_mesh(pnext_SOM,vnext_SOM, ...
                                                  SOMsz, COMsz);
...
%}
%{
opti.set_initial(COM.stiffness, [-300 -10 -300]);
opti.set_initial(COM.damping, [-4 -2 -4]);
opti.set_initial(COM.z_sum, 0.03);
%}
%{x
opti.set_initial(COM.stiffness, [-300.38 -13.47 -269.82]);
opti.set_initial(COM.damping, [-4.48 -3.08 -4.33]);
opti.set_initial(COM.z_sum, 0.0273);
%}
sol = opti.solve();
