...
results = CL_SimulationFcn(NTraj, Ts, Hp, W_Q, W_R);
store_state = results.SOMstate;

Ref_l = load(['required_files\trajectories\ref_',num2str(NTraj),'L.csv']);
Ref_r = load(['required_files\trajectories\ref_',num2str(NTraj),'R.csv']);
nPtRef = size(Ref_l,1);
